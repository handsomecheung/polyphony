...
The router runs the core of umbilical's network control logic. All containers use `network_mode: host` to avoid NetworkManager DNS conflicts on the Raspberry Pi 64-bit OS. The stack is deployed via `local/router/sync.sh` (rsync + `my-secret render` to host `r3b`) and managed with `restart.sh` / `stop.sh`.
...
`my-build-image.sh` reads the private registry host from `my-secret`, creates a named buildx builder, rsyncs sources to a temporary build directory under `/mnt/coder-sharepoint/build-image/`, and removes the directory after a successful push.
...
- `my-secret` CLI for rendering `__{{...}}__` placeholders
...
- [../koishi/README.md](../koishi/README.md) — Kubernetes cluster whose services are exposed through these tunnels
