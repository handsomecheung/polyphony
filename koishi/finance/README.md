...
```
finance/
├── k8s.namespace.yaml          # Namespace definition (finance)
├── deploy.sh                   # Orchestrates deployment of all components
├── seek/                       # Query service
│   ├── code/
│   │   ├── main.py            # FastAPI service with /ok, /time, /latest endpoints
│   │   └── requirements.txt    # FastAPI, uvicorn, psycopg2-binary, pytz, pydantic
│   ├── Dockerfile             # python:3.12-slim base image
│   ├── build.sh               # Builds cloudpublic/finance/seek:latest
│   ├── deploy.sh              # Deploys seek Deployment, Service, dual Ingress routes
│   └── k8s.app.yaml           # Deployment, Service, Ingress, IngressRoute, Certificate
└── tempo/                      # Scheduled data fetcher
    ├── code/
    │   ├── main.py            # Fetches Yahoo Finance data daily at 6am UTC
    │   ├── run.sh             # Local test runner
    │   └── requirements.txt    # yfinance, psycopg2-binary, pytz, pandas
    ├── Dockerfile             # python:3.12-slim base image
    ├── build.sh               # Builds cloudpublic/finance/tempo:latest
    ├── deploy.sh              # Deploys tempo CronJob
    ├── env.sample             # Example environment variables
    └── k8s.app.yaml           # CronJob definition
```
...
**k8s.namespace.yaml**: Creates the `finance` namespace to isolate finance services from other cluster workloads.
