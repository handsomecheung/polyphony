...
if ! tmux has-session -t "${session}" 2>/dev/null; then
	...
	tmux new -d -A -s "${session}" bash -c "timeout 600 kubectl drain --delete-emptydir-data --ignore-daemonsets ${node} && systemctl poweroff || { echo 'ERROR: kubectl drain failed. Shutdown aborted!'; tail -f /dev/null; }"
fi
